...
if ($curPassword !== false && Accounts::CheckPassword($GLOBALS["accountID"], $curPassword))
{	
	if (($confirmPassword !== false && $confirmPassword === $newPassword) ||
		($confirmPassword === false && $newPassword !== false))
	{
		Accounts::ChangePassword($GLOBALS["accountID"], $newPassword);
		header("Location: /myhub/index.php");
...
if ($successfulChange)
{
	$headContent .= "<meta http-equiv='Refresh' content='3; URL='/myhub/index.php'>";
}
